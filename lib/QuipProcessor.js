...
class QuipProcessor {
        ...
        for(const message of messages) {
            ...
            if(message.annotation && message.annotation.id) {
                if(message.annotation.highlight_section_ids) {
                  ...
                  if(message.annotation.highlight_section_ids.length > 1) {
                      console.log(`Annotation with multiple sections for ${quipThread.thread.title}!`, message);
                  }
                } else {
    ...
    async _processDocumentThread(quipThread, path) {
        ...
        threadHtml += await this._threadMetadata(quipThread);
    ...
    async _threadMetadata(quipThread) {
        const createdAt = new Date(quipThread.thread.created_usec / 1000.0);
        const updatedAt = new Date(quipThread.thread.updated_usec / 1000.0);
        const author = await this._findReferencedUser({
          referenceId: quipThread.thread.author_id,
        });
        const authorName =
              author?.name || `Unknown (${quipThread.thread.author_id})`;
        const html = `<h2>Metadata</h3>
<h3>Author</h3>
<p>${authorName}</p>
<h3>Created</h3>
<p>${createdAt}</p>
...
<p>${updatedAt}</p>
<h3>Original Link</h3>
<p>${quipThread.thread.link}</p>
<h3>Document ID</h3>
<p>${quipThread.thread.document_id}</p>
`;
        return html;
    }
